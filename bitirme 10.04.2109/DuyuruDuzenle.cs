...
namespace bitirme
{
    public partial class DuyuruDuzenle : Form
    {
        ...
        int a;
        public DuyuruDuzenle(String isim,int min,string date)
        {
            date2 = date;
            min2 = min;
            InitializeComponent();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                int c = 0;
                a = int.Parse(numericUpDown1.Text.ToString());
                String b = richTextBox1.Text.ToString();
                ...
                cmd.CommandText = "Select D.DuyuruId from eğitmen E,duyuru D Where D.DuyuruId = '" + a + "' and E.EgitmenId = '" + EgitmenId + "'and D.EgitmenId=E.EgitmenId";
                object val = cmd.ExecuteScalar();
                if(val==null || val == DBNull.Value)             
                {
                    MessageBox.Show("Belirtilen Id de duyuru yok veya silinmek istenen Id duyurusu size ait değil");
                }
                else
                {
                    cmd.CommandText = "UPDATE duyuru SET `DuyuruAciklama` = '" + b + "' WHERE (`DuyuruId` = '" + a + "') and (`EgitmenId` = '" + EgitmenId + "')";
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Güncellenmiştir.");
                }
            }catch(Exception ex)
            {
                MessageBox.Show("Güncellenememiştir.");
            }
            this.Dispose();
            ...
            try
            {
                ...
                if (val == null || val==DBNull.Value)
                {
                    ...
                    if (a > a2)
                    {
                        ...
                        {
                            MessageBox.Show("Gösterilecek Daha düşük ID ye sahip duyuru yoktur.!!");
                        }
                        else
                        {
                            a = int.Parse(cmd.ExecuteScalar().ToString());
                    ...
                    else if (a < a2)
                    {
                        ...
                        {
                            MessageBox.Show("Gösterilecek Daha yüksek ID ye sahip duyuru yoktur.!!");
                        }
                        else
                        {
                            a = int.Parse(cmd.ExecuteScalar().ToString());
